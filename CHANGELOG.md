...
- Added `xtask check-first-use-by-surface-guide` to prove the Rust, CLI, and
  server routes from the first-use routing guide while keeping Python public
  registry proof separate.
...
- Recorded the blocked public Python registry install-back proof commands in
  the machine-readable evidence parity manifest.
