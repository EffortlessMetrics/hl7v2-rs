...
fn test_ack_swaps_sending_receiving_applications() {
    let original_message =
        parse(b"MSH|^~\\&|AppA|FacA|AppB|FacB|20250128152312||ADT^A01|MSG123|P|2.5.1\r").unwrap();
...
fn test_ack_preserves_processing_id() {
    let original_message =
        parse(b"MSH|^~\\&|AppA|FacA|AppB|FacB|20250128152312||ADT^A01|MSG123|T|2.5.1\r").unwrap();
...
fn test_ack_with_error_none() {
    ...
    let ack_message = ack_with_error(&original_message, AckCode::AE, None).unwrap();
...
fn test_err_segment_contains_error_message() {
    ...
    let ack_message = ack_with_error(&original_message, AckCode::AE, Some(error_msg)).unwrap();
...
fn test_ack_with_minimal_message() {
    let original_message = parse(b"MSH|^~\\&|||||||ACK|||2.5.1\r").unwrap();
...
fn test_ack_with_empty_fields() {
    let original_message =
        parse(b"MSH|^~\\&|AppA||AppB|||20250128152312||ADT^A01|MSG123|P|2.5.1\r").unwrap();
...
fn test_ack_with_missing_optional_fields() {
    let original_message =
        parse(b"MSH|^~\\&|AppA|FacA|AppB|FacB|20250128152312||ADT^A01|MSG123\r").unwrap();
...
fn test_ack_with_long_control_id() {
    ...
    let original_message = parse(
        format!(
            "MSH|^~\\&|AppA|FacA|AppB|FacB|20250128152312||ADT^A01|{}|P|2.5.1\r",
            long_id
        )
        .as_bytes(),
    )
...
fn test_ack_with_unicode_in_fields() {
    let original_message =
        parse("MSH|^~\\&|AppÄ|Facö|Recvß|Facü|20250128152312||ADT^A01|MSG123|P|2.5.1\r".as_bytes())
            .unwrap();
...
fn test_err_segment_structure() {
    ...
    let ack_message =
        ack_with_error(&original_message, AckCode::AE, Some("Test error message")).unwrap();
...
fn test_get_field_value_empty_field() {
    let original_message =
        parse(b"MSH|^~\\&|AppA||AppB|FacB|20250128152312||ADT^A01|MSG123|P|2.5.1\r").unwrap();
