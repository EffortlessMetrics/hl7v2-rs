...
fn test_ack_reject_invalid_message_type() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||UNKNOWN^TYPE|MSG12345|P|2.5.1\r")
            .unwrap();

    let ack_msg = ack_with_error(&original, AckCode::AR, Some("Unknown message type")).unwrap();
...
fn test_ack_roundtrip_with_writer() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||ADT^A01|MSG12345|P|2.5.1\r")
            .unwrap();
    ...
    assert_eq!(
        std::str::from_utf8(&parsed_ack.segments[0].id).unwrap(),
        "MSH"
    );
    assert_eq!(
        std::str::from_utf8(&parsed_ack.segments[1].id).unwrap(),
        "MSA"
    );
}
...
fn test_ack_for_hl7_23() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||ADT^A01|MSG123|P|2.3\r").unwrap();
...
fn test_ack_for_hl7_251() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||ADT^A01|MSG123|P|2.5.1\r").unwrap();
...
fn test_ack_for_hl7_27() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||ADT^A01|MSG123|P|2.7\r").unwrap();
...
fn test_ack_preserves_training_mode() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||ADT^A01|MSG123|T|2.5.1\r").unwrap();
...
fn test_ack_preserves_debugging_mode() {
    let original =
        parse(b"MSH|^~\\&|APP|FAC|RECV|RECVFAC|20250128150000||ADT^A01|MSG123|D|2.5.1\r").unwrap();
...
fn test_ack_for_minimal_message() {
    let minimal = parse(b"MSH|^~\\&|||||||ACK|||2.5.1\r").unwrap();
