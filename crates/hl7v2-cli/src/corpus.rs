//! Corpus subcommands for summaries, fingerprints, and diffs.
...
pub(crate) fn handle_command(command: &CorpusCommands) -> Result<(), Box<dyn std::error::Error>> {
    match command {
...
pub(crate) fn corpus_summarize_command(
    path: &PathBuf,
    format: &ReportFormat,
    schema_version: u8,
...
pub(crate) fn corpus_diff_command(
    before: &PathBuf,
    after: &PathBuf,
    profile: Option<&PathBuf>,
    format: &ReportFormat,
...
pub(crate) fn corpus_fingerprint_command(
    path: &PathBuf,
    profile: Option<&PathBuf>,
    format: &ReportFormat,
    schema_version: u8,
