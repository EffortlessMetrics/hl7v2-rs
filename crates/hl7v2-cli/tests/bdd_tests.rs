...
fn given_cli_available(_world: &mut CliWorld) {
    ...
    assert!(binary_path.exists() || binary_path.with_extension("exe").exists(),
        "CLI binary should exist at {:?}", binary_path);
}
...
fn given_large_file(world: &mut CliWorld) {
    ...
    let mut content = String::from("MSH|^~\\&|App|Fac|Recv|Fac|20250101000000||ADT^A01|MSG001|P|2.5.1\r");
    for i in 1..=100 {
...
fn when_run_with_io(world: &mut CliWorld, command: String) {
    let input = world.input_file.as_ref().expect("Input file should be set");
    let output = world.output_file.as_ref().expect("Output file should be set");
...
fn then_succeed(world: &mut CliWorld) {
    assert!(world.command_succeeded(), 
        "Command should succeed but failed with stderr: {}", world.stderr());
}

#[then("the command should fail")]
fn then_fail(world: &mut CliWorld) {
    assert!(!world.command_succeeded(), 
        "Command should fail but succeeded with stdout: {}", world.stdout());
}

#[then("the command should fail with non-zero exit code")]
fn then_fail_nonzero(world: &mut CliWorld) {
    let code = world.exit_code();
    assert!(code.map(|c| c != 0).unwrap_or(true),
        "Command should have non-zero exit code but was {:?}", code);
}
...
fn then_valid_json(world: &mut CliWorld) {
    ...
    assert!(result.is_ok(), "Output should be valid JSON but got: {}", stdout);
}
...
fn then_contains_segment(world: &mut CliWorld, segment: String) {
    ...
    assert!(stdout.contains(&segment), 
        "Output should contain segment {} but got: {}", segment, stdout);
}
...
fn then_contains(world: &mut CliWorld, text: String) {
    ...
    assert!(stdout.contains(&text), 
        "Output should contain '{}' but got: {}", text, stdout);
}
...
fn then_error_contains_error(world: &mut CliWorld) {
    ...
    assert!(stderr.contains("Error"), 
        "Stderr should contain 'Error' but got: {}", stderr);
}

#[then("the error should indicate file not found")]
fn then_error_file_not_found(world: &mut CliWorld) {
    let stderr = world.stderr();
    assert!(stderr.contains("Error") || stderr.contains("not found") || stderr.contains("No such"),
...
fn then_error_unknown_command(world: &mut CliWorld) {
    ...
    assert!(stderr.contains("error") || !stderr.is_empty(),
        "Error should indicate unknown command: {}", stderr);
}
...
fn then_validation_issues(world: &mut CliWorld) {
    ...
    let stderr = world.stderr();
    assert!(
        output.contains("issues") || output.contains("Error") || 
        stderr.contains("issues") || stderr.contains("Error"),
...
fn then_mllp_start_block(world: &mut CliWorld) {
    if let Some(Ok(output)) = &world.result {
        ...
        assert_eq!(output.stdout[0], 0x0B, "Output should start with MLLP start block (0x0B)");
    } else {
...
fn then_valid_ack(world: &mut CliWorld) {
    ...
    assert!(msg.segments.iter().any(|s| &s.id == b"MSA"), "ACK should contain MSA segment");
}
...
fn then_output_has_messages(world: &mut CliWorld) {
    ...
    let count = std::fs::read_dir(output_dir)
        ...
        .unwrap_or(0);
    assert!(count > 0, "Output directory should contain at least one message");
}
...
fn then_output_has_n_messages(world: &mut CliWorld, count: usize) {
    ...
    assert!(actual_count >= count, 
        "Output directory should contain at least {} messages but has {}", count, actual_count);
}
...
fn then_gen_stats(world: &mut CliWorld) {
    ...
    assert!(
        ...
        "Output should contain generation statistics: {}", stdout
    );
}
