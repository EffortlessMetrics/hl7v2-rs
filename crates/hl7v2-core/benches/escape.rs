...
/// Create sample text that needs no unescaping (clean)
fn create_sample_clean_escaped_text() -> String {
    "This is a test with no escape sequences needed whatsoever".to_string()
}
...
/// Create sample text that needs no escaping (clean)
fn create_sample_clean_unescaped_text() -> String {
    "This is a test with no special characters needed whatsoever".to_string()
}

/// Benchmark unescaping text
fn bench_unescape_text(c: &mut Criterion) {
    let dirty_text = create_sample_escaped_text();
    let clean_text = create_sample_clean_escaped_text();
    ...
    group.bench_function("dirty", |b| {
        b.iter(|| {
            let result = unescape_text(black_box(&dirty_text), black_box(&delims));
            black_box(result)
        })
    });
...
fn bench_escape_text(c: &mut Criterion) {
    let dirty_text = create_sample_unescaped_text();
    let clean_text = create_sample_clean_unescaped_text();
    let delims = Delims::default();
    ...
    group.bench_function("dirty", |b| {
        b.iter(|| {
            let result = escape_text(black_box(&dirty_text), black_box(&delims));
            black_box(result)
        })
    });
