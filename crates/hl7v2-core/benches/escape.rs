...
/// Create sample clean text (no special chars) for benchmarking
fn create_sample_clean_text() -> String {
    "This is a test with no special characters and should be faster".to_string()
}
...
fn bench_escape_text(c: &mut Criterion) {
    ...
    group.bench_function("with_delims", |b| {
        b.iter(|| {
    ...
    group.bench_function("no_delims", |b| {
        b.iter(|| {
