...
pub fn unescape_text(text: &str, delims: &Delims) -> Result<String, Error> {
    // Fast path: if no escape character is present, return the original string
...
pub fn escape_text(text: &str, delims: &Delims) -> String {
    // Fast path: if no delimiters are present, return the original string
    // This avoids unnecessary allocation (3x) and iteration for the common case
