...
pub fn unescape_text(text: &str, delims: &Delims) -> Result<String, Error> {
    // Optimization: Check if any characters need unescaping before allocating
...
pub fn escape_text(text: &str, delims: &Delims) -> String {
    // Optimization: Check if any characters need escaping before allocating
    // Using a slice of chars is significantly faster than a closure
    let delims_arr = [delims.field, delims.comp, delims.rep, delims.esc, delims.sub];
    if !text.contains(&delims_arr[..]) {
