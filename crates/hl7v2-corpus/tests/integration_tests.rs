...
fn test_full_corpus_workflow() {
    ...
    // Verify splits
    let total_split: usize =
        parsed.splits.train.len() + parsed.splits.validation.len() + parsed.splits.test.len();
...
fn test_splits_different_seeds_different_distributions() {
    ...
    assert!(
        !all_same,
        "All splits are identical despite different seeds"
    );
}
