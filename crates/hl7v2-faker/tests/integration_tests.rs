...
fn test_full_patient_demographics() {
    ...
    // DOB is generated but may not appear directly in the PID segment format
    let _ = dob; // Use dob to avoid warning
}
...
fn test_gender_specific_names() {
    ...
    // Generate male names
    let male_names: std::collections::HashSet<String> =
        (0..50).map(|_| faker.name(Some("M"))).collect();
    ...
    let female_names: std::collections::HashSet<String> =
        (0..50).map(|_| faker.name(Some("F"))).collect();
