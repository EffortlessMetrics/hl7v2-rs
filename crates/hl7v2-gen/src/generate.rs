...
use crate::delimiters::parse_delimiters;
use crate::template::{Template, ValueSource};
...
pub(crate) fn generate_single_message(
    template: &Template,
    rng: &mut StdRng,
    _index: usize,
...
fn generate_value(value_source: &ValueSource, rng: &mut StdRng) -> Result<String, Error> {
    match value_source {
        ...
        ValueSource::Gaussian { mean, sd, precision } => {
            ...
            let value = rng
                .sample(rand_distr::Normal::new(*mean, *sd).map_err(|_| Error::InvalidEscapeToken)?);
        ...
        ValueSource::RealisticAddress => {
            ...
            let cities = &[
                "Anytown", "Springfield", "Riverside", "Fairview", "Centerville", "Georgetown",
                "Mount Pleasant", "Oakland", "Middletown", "Franklin",
            ];
        ...
        ValueSource::RealisticEthnicity => {
            let ethnicities = &["Hispanic or Latino", "Not Hispanic or Latino", "Declined to Specify"];
