...
async fn test_server_handles_multiple_concurrent_connections() {
    ...
    assert!(
        message_count >= 5,
        "Should have handled at least 5 messages, got {}",
        message_count
    );
}
...
async fn test_mllp_codec_handles_multiple_messages_in_buffer() {
    ...
    codec
        .encode(BytesMut::from(b"MSG1".as_slice()), &mut buffer)
        .unwrap();
    codec
        .encode(BytesMut::from(b"MSG2".as_slice()), &mut buffer)
        .unwrap();
    codec
        .encode(BytesMut::from(b"MSG3".as_slice()), &mut buffer)
        .unwrap();
