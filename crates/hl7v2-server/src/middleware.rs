...
pub async fn auth_middleware(
    ...
    let expected_key = match &state.api_key {
        Some(key) => key,
        None => {
            // No key configured - fail closed for security
            tracing::error!("Server configured with no API key - refusing all requests to protected endpoints");
...
mod tests {
    ...
    fn test_constant_time_eq() {
        ...
        assert!(!constant_time_eq("secret", "secreT"));
        assert!(!constant_time_eq("secret", "supersecret"));
