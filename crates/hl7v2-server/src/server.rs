...
impl Server {
    ...
    pub fn new(config: ServerConfig) -> Self {
        ...
        // Determine API key - prefer config, fall back to env var
        let api_key = config.api_key.clone().or_else(|| {
            std::env::var("HL7V2_API_KEY").ok().filter(|k| !k.is_empty())
        });
...
mod tests {
    ...
    fn test_server_builder() {
        ...
        assert!(server.config.api_key.is_none());
    }
    ...
    #[test]
    fn test_server_builder_with_api_key() {
        let server = Server::builder()
            .api_key("secret-key")
            .build();

        assert_eq!(server.config.api_key.as_deref(), Some("secret-key"));
        assert_eq!(server.state.api_key.as_deref(), Some("secret-key"));
    }
}
