...
impl Server {
    ...
    pub fn new(config: ServerConfig) -> Self {
        ...
        // Get API key from config or environment, or fail if missing
        let api_key = config.api_key.clone().unwrap_or_else(|| {
            std::env::var("HL7V2_API_KEY").unwrap_or_else(|_| {
                tracing::warn!("HL7V2_API_KEY not set, defaulting to empty string (auth may fail)");
                String::new()
            })
        });
