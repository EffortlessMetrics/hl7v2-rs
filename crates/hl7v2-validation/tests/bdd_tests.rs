...
use hl7v2_validation::{
    check_rule_condition, Issue, RuleCondition, Severity,
};
...
impl ValidationWorld {
    ...
    fn add_error(&mut self, code: &str, path: &str, detail: &str) {
        self.issues.push(Issue::error(code, Some(path.to_string()), detail.to_string()));
        self.validation_passed = false;
    }
    
    fn add_warning(&mut self, code: &str, path: &str, detail: &str) {
        self.issues.push(Issue::warning(code, Some(path.to_string()), detail.to_string()));
    }
}
...
fn given_adt_a01_all_fields(world: &mut ValidationWorld) {
    world.message_content = Some(concat!(
...
fn given_profile_sex_table_0001(world: &mut ValidationWorld) {
    world.profile_constraints.insert(
        ...
        ProfileConstraint {
            ...
            required: false,
            severity: Severity::Error,
            allowed_values: Some(vec!["M".to_string(), "F".to_string(), "O".to_string(), "U".to_string(), "A".to_string(), "N".to_string()]),
            pattern: None,
            range_min: None,
            range_max: None,
            luhn_checksum: false,
    ...
    if let Some(msg) = &msg_clone {
        ...
        for field in required_fields {
            ...
            if !check_rule_condition(msg, &condition) {
                issues_to_add.push(Issue::error("MISSING_REQUIRED_FIELD", Some(field.to_string()), format!("{} is required", field)));
            }
        }
        ...
        for (field, constraint) in &world.profile_constraints {
            ...
            if let Some(allowed) = &constraint.allowed_values {
                ...
                if !check_rule_condition(msg, &condition) {
                    issues_to_add.push(Issue::error("INVALID_CODE_VALUE", Some(field.to_string()), format!("Invalid value for {}", field)));
                }
            }
        }
    ...
    if let Some(msg) = &world.parsed_message {
        ...
        if pid_count > 1 {
            world.add_error(
                ...
                "PID",
                &format!("Message contains {} PID segments, but maximum allowed is 1", pid_count)
            );
        }
    }
}
...
fn then_validation_succeeds(world: &mut ValidationWorld) {
    assert!(world.validation_passed, "Validation should have succeeded but failed with issues: {:?}", world.issues);
}

#[then("validation should fail")]
fn then_validation_fails(world: &mut ValidationWorld) {
    assert!(!world.validation_passed, "Validation should have failed but passed");
}

#[then("there should be 0 errors")]
fn then_zero_errors(world: &mut ValidationWorld) {
    let error_count = world.issues.iter().filter(|i| i.severity == Severity::Error).count();
...
fn then_one_warning(world: &mut ValidationWorld) {
    let warning_count = world.issues.iter().filter(|i| i.severity == Severity::Warning).count();
    assert_eq!(warning_count, 1, "Expected 1 warning, found {}", warning_count);
}

#[then("the error code should be \"MISSING_REQUIRED_FIELD\"")]
fn then_error_code_missing_required(world: &mut ValidationWorld) {
    let has_code = world.issues.iter().any(|i| i.code == "MISSING_REQUIRED_FIELD");
    assert!(has_code, "Expected error code 'MISSING_REQUIRED_FIELD' not found");
}
...
fn then_error_code_invalid_data_type(world: &mut ValidationWorld) {
    ...
    assert!(has_code, "Expected error code 'INVALID_DATA_TYPE' not found");
}

#[then("the error code should be \"CHECKSUM_VALIDATION_FAILED\"")]
fn then_error_code_checksum(world: &mut ValidationWorld) {
    let has_code = world.issues.iter().any(|i| i.code == "CHECKSUM_VALIDATION_FAILED");
    assert!(has_code, "Expected error code 'CHECKSUM_VALIDATION_FAILED' not found");
}

#[then("the error code should be \"CARDINALITY_VIOLATION\"")]
fn then_error_code_cardinality(world: &mut ValidationWorld) {
    let has_code = world.issues.iter().any(|i| i.code == "CARDINALITY_VIOLATION");
    assert!(has_code, "Expected error code 'CARDINALITY_VIOLATION' not found");
}

#[then("the error should reference field \"PID.3\"")]
fn then_error_references_pid3(world: &mut ValidationWorld) {
    let references_field = world.issues.iter().any(|i| i.path.as_deref() == Some("PID.3") || i.path.as_deref() == Some("PID.3.1"));
    assert!(references_field, "Expected error referencing field 'PID.3' not found");
}

#[then("the error should reference field \"PID.5\"")]
fn then_error_references_pid5(world: &mut ValidationWorld) {
    let references_field = world.issues.iter().any(|i| i.path.as_deref() == Some("PID.5") || i.path.as_deref() == Some("PID.5.1"));
    assert!(references_field, "Expected error referencing field 'PID.5' not found");
}

#[then("validation should succeed for PID.7")]
fn then_validation_succeeds_for_pid7(world: &mut ValidationWorld) {
    assert!(world.last_field_valid.unwrap_or(true), "Field validation should have succeeded");
}

#[then("the error should indicate \"exceeds maximum length\"")]
fn then_error_indicates_length(world: &mut ValidationWorld) {
    let has_length_error = world.issues.iter().any(|i| 
        i.detail.contains("length") || i.code == "FIELD_LENGTH_EXCEEDED"
    );
...
fn then_error_lists_values(world: &mut ValidationWorld) {
    let has_values = world.issues.iter().any(|i| 
        i.detail.contains("M") || i.code == "INVALID_CODE_VALUE"
    );
...
fn then_error_references_conditional(world: &mut ValidationWorld) {
    let has_conditional = world.issues.iter().any(|i| 
        i.code == "CROSS_FIELD_VALIDATION_FAILED"
    );
    assert!(has_conditional, "Expected conditional validation error not found");
}

#[then("the error should indicate \"birth date after message timestamp\"")]
fn then_error_birth_after_msg(world: &mut ValidationWorld) {
    let has_temporal_error = world.issues.iter().any(|i| 
        i.code == "TEMPORAL_VALIDATION_FAILED"
    );
    assert!(has_temporal_error, "Expected temporal validation error not found");
}

#[then("the error should indicate \"below minimum\"")]
fn then_error_below_minimum(world: &mut ValidationWorld) {
    assert!(!world.validation_passed, "Validation should have failed for below minimum");
}

#[then("the error should indicate \"above maximum\"")]
fn then_error_above_maximum(world: &mut ValidationWorld) {
    assert!(!world.validation_passed, "Validation should have failed for above maximum");
}

#[then("the error should indicate \"EVN must appear before PID\"")]
fn then_error_segment_order(world: &mut ValidationWorld) {
    let has_order_error = world.issues.iter().any(|i| 
        i.detail.contains("EVN") && i.detail.contains("PID")
    );
...
fn then_error_empty_message(world: &mut ValidationWorld) {
    assert!(!world.validation_passed, "Validation should have failed for empty message");
}

#[then("the error should indicate \"missing MSH segment\"")]
fn then_error_missing_msh(world: &mut ValidationWorld) {
    assert!(!world.validation_passed, "Validation should have failed for missing MSH");
}

#[then("the error should indicate \"unknown message type\"")]
fn then_error_unknown_type(world: &mut ValidationWorld) {
    assert!(world.parsed_message.is_some(), "Message should have been parsed");
}
...
fn then_all_validations_succeed(world: &mut ValidationWorld) {
    assert!(world.validation_passed, "All validations should have succeeded");
}
...
fn then_2_validations_succeed(world: &mut ValidationWorld) {
    ...
    assert_eq!(actual, 2, "Expected 2 successful validations, found {}", actual);
}
...
fn then_loading_fails(world: &mut ValidationWorld) {
    assert!(!world.validation_passed, "Profile loading should have failed");
}
...
fn then_1_error_1_warning(world: &mut ValidationWorld) {
    let errors = world.issues.iter().filter(|i| i.severity == Severity::Error).count();
    let warnings = world.issues.iter().filter(|i| i.severity == Severity::Warning).count();
