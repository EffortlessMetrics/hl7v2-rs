//! Profile data model and report types.

use super::super::validation::{self, RuleAction, RuleCondition};
...
impl ProfileLintReport {
    pub(crate) fn from_issues(issues: Vec<ProfileLintIssue>) -> Self {
...
pub struct ProfileTestCaseReport {
    ...
    pub validation_report: Option<validation::ValidationReport>,
...
pub(crate) enum ExpectedReportCandidate {
    File(PathBuf),
    Ambiguous(PathBuf),
}
...
impl ProfileLintIssue {
    pub(crate) fn error(code: &str, path: Option<String>, message: String) -> Self {
        Self {
            code: code.to_string(),
    ...
    pub(crate) fn warning(code: &str, path: Option<String>, message: String) -> Self {
        Self {
            code: code.to_string(),
