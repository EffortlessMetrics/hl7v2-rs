...
release-readiness workflow, and the focused SRP module split train through #691.
...
| Corpus observability | ✅ Stable | `corpus summarize`, `corpus fingerprint`, and `corpus diff` produce feed-level evidence for regression and migration review. Current proof includes shared dirty-corpus coverage for Z-segments, legacy MSH/encoding fields, generated MLLP bytes, large OBX expansion, malformed delimiters, partial batch-like input, and safe parse-error output across Rust core, CLI, REST server, and gRPC server surfaces. |
...
and the post-release SRP refresh at `4cf501ddc0f7fc3d027b3ce2459e899fe4aa7092`.
