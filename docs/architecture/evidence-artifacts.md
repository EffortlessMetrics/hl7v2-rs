...
| Redaction output | `hl7v2 redact --format json`, Python `redact()` | `hl7v2::redact::SafeAnalysisRedactionOutput` for Python; CLI-local wrapper still matches the same fields | Has input and policy SHA-256 values plus a nested receipt. JSON Schema exists at `schemas/evidence/safe-analysis-redaction-output-v1.schema.json` and validates both current nested receipt forms. A target v2 schema exists for future top-level provenance; live producers still emit the v1-compatible outer shape. |
