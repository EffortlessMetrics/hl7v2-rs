...
| Redaction receipt | `hl7v2 redact --format json`, bundle `redaction-receipt.json`, server `/hl7/validate-redacted` | CLI/server-local `RedactionReceipt` shapes | Captures actions and PHI removal status; JSON Schema exists at `schemas/evidence/redaction-receipt-v1.schema.json`; no `schema_version`; no `tool_version`. |
...
| Evidence bundle summary | `hl7v2 bundle ...` stdout; server `/hl7/bundle` response | CLI/server-local `EvidenceBundleSummary` | Has `bundle_version`; JSON Schema exists at `schemas/evidence/evidence-bundle-v1.schema.json`; no `tool_version`; includes `README.md` and `manifest.json` in the artifact list. Server reports the configured-root-relative bundle id, not an absolute filesystem path. |
...
| Evidence bundle manifest | Bundle `manifest.json` | CLI/server-local `EvidenceBundleManifest` | Records `bundle_version`, `tool_name`, `tool_version`, bundle-relative artifact paths, roles, and SHA-256 hashes; JSON Schema exists at `schemas/evidence/evidence-bundle-manifest-v1.schema.json`; replay verifies manifest catalog and hashes before using artifacts. |
| Evidence bundle environment | Bundle `environment.json` | CLI/server-local `EvidenceBundleEnvironment` | Has `bundle_version`, `tool_name`, `tool_version`, input/profile/policy hashes, and replay command. |
...
| Python | Exposes parse, JSON conversion, normalize, validation report dict/JSON parity, and corpus summary/fingerprint/diff dict parity. It does not yet expose redaction, bundle, or replay artifacts. |
