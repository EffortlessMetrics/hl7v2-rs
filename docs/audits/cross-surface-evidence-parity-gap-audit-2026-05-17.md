...
| corpus summary / fingerprint / diff | Rust core, CLI, REST, gRPC, and local Python dirty-corpus parity proof share `test_data/dirty-real-world/`; `cargo run -p xtask -- check-dirty-corpus-parity` composes the Rust/CLI/REST/gRPC acceptance path and now includes CLI, REST, gRPC, and optional local-wheel Python validate/redact/bundle/replay workflows over the shared Z-segment fixture. | TypeScript is unimplemented; Python remains local-wheel proof until registry install-back exists. | Keep Python dirty workflow proof local-wheel-scoped until TestPyPI/PyPI install-back exists; TypeScript/WASM later. |
...
10. Keep the local-wheel Python dirty validate/redact/bundle/replay smoke
