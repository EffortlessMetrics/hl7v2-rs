...
| corpus summary / fingerprint / diff | Rust core, CLI, REST, gRPC, and local Python dirty-corpus parity proof share `test_data/dirty-real-world/`; `cargo run -p xtask -- check-dirty-corpus-parity` composes the Rust/CLI/REST/gRPC acceptance path and now includes CLI, REST, and gRPC validate/redact/bundle/replay workflows over the shared Z-segment fixture. | TypeScript is unimplemented; Python dirty-corpus workflow proof beyond corpus commands remains focused follow-up work; Python remains local-wheel proof until registry install-back exists. | Extend dirty-corpus validate/redact/bundle/replay workflow proof to Python only after public package proof or as a focused local-wheel test PR. |
...
10. Extend dirty-corpus validate/redact/bundle/replay proof to Python only
   after public package proof or as a focused local-wheel test PR.
