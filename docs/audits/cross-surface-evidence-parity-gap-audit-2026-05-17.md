...
| corpus summary / fingerprint / diff | Rust core, CLI, REST, gRPC, and local Python dirty-corpus parity proof share `test_data/dirty-real-world/`; `cargo run -p xtask -- check-dirty-corpus-parity` composes the Rust/CLI/REST/gRPC acceptance path and now includes one CLI validate/redact/bundle/replay workflow over the shared Z-segment fixture. | TypeScript is unimplemented; REST, gRPC, and Python dirty-corpus workflow proof beyond corpus commands remains focused follow-up work; Python remains local-wheel proof until registry install-back exists. | Extend dirty-corpus validate/redact/bundle/replay workflow proof to REST, gRPC, and Python only in focused test PRs. |
...
9. Keep `cargo run -p xtask -- check-dirty-corpus-parity` as the shared
   ...
   summary/fingerprint/diff proof plus the CLI validate/redact/bundle/replay
   workflow proof; use `--include-python` only when a local Python wheel is
   installed.
10. Extend dirty-corpus validate/redact/bundle/replay proof to another surface
   only where the next surface has a concrete user workflow.
