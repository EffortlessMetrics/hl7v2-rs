...
| corpus summary / fingerprint / diff | Rust core, CLI, REST, gRPC, and local Python dirty-corpus parity proof share `test_data/dirty-real-world/`; `cargo run -p xtask -- check-dirty-corpus-parity` composes the Rust/CLI/REST/gRPC acceptance path and now includes CLI and REST validate/redact/bundle/replay workflows over the shared Z-segment fixture. | TypeScript is unimplemented; gRPC and Python dirty-corpus workflow proof beyond corpus commands remains focused follow-up work; Python remains local-wheel proof until registry install-back exists. | Extend dirty-corpus validate/redact/bundle/replay workflow proof to gRPC and Python only in focused test PRs. |
...
9. Keep `cargo run -p xtask -- check-dirty-corpus-parity` as the shared
   ...
   summary/fingerprint/diff proof plus the CLI and REST
