...
| Bundle artifacts | CLI, Python, and server bundles write `message.redacted.hl7`, `validation-report.json`, `field-paths.json`, `profile.yaml`, `redaction-receipt.json`, `environment.json`, `replay.sh`, `replay.ps1`, `README.md`, and `manifest.json`. | Bundle README is generated and manifest-hashed. CLI and Python bundles can opt into v2 `field-paths.json`, `redaction-receipt.json`, `environment.json`, and `manifest.json` artifacts with bundle `schema_version = 2`; defaults and server-created bundles remain v1-compatible. Profile text is user-authored and included as supplied. |
