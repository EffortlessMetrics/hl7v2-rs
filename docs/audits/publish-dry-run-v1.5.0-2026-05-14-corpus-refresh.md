# v1.5.0 Corpus Refresh Publish Dry-Run Receipt

Date: 2026-05-14
Branch: `release/refresh-v1.5-readiness-after-corpus-proof`
Commit SHA: `425ff79b959ef5ceff1bdd072cbe074ff2a7ab04`
Version: `1.5.0`
Result: Passed

This receipt refreshes the non-publishing v1.5.0 release-readiness proof after
the dirty real-world corpus proof landed on `main`.
...
This refresh covers the current release candidate after #625 added the focused
dirty-corpus proof for core corpus summary, fingerprint, and diff behavior.
That proof covers Z-segments, odd MSH fields, MLLP-framed bytes, larger
OBX-heavy messages, malformed delimiters, and safe parse-error output.
...
| `CARGO_TARGET_DIR=F:\cargo-target\hl7v2-rs-readiness-corpus-refresh CARGO_INCREMENTAL=0 cargo +1.95.0 run -p xtask -- publish-dry-run --surface primary --workspace-patches --allow-dirty` | pass; `hl7v2`, `hl7v2-server`, and `hl7v2-cli` packaged and verified; upload aborted because this was a dry-run |
| `CARGO_TARGET_DIR=F:\cargo-target\hl7v2-rs-readiness-corpus-refresh CARGO_INCREMENTAL=0 PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 cargo +1.95.0 run -p xtask -- publish-dry-run --surface bindings --workspace-patches --allow-dirty` | pass; `hl7v2-python` package files were listed, then the crate packaged and verified; upload aborted because this was a dry-run |
...
| `cargo +1.95.0 run -p xtask -- check-doc-links` | pass; 156 Markdown files and 351 local links checked |
| `cargo +1.95.0 run -p xtask -- check-file-policy` | pass; 515 tracked/untracked non-ignored files checked against 39 allowlist entries and 30 companion ledger entries |
| `git diff --check` | pass |
