# v1.5.0 Numeric Validation Readiness Refresh - 2026-05-15

This receipt refreshes the v1.5.0 release-readiness proof after the nightly
property-test command repair and finite numeric validation fix landed.
...
Refreshed commit:
`b0018c8760f07d738b3a6b7a11eeeda300786ef2`.

Changes included since the prior gRPC corpus evidence refresh:

- #632 fixed the nightly property-test command so it runs the existing
  `hl7v2` property tests instead of a non-existent workspace integration test
  target.
- #633 rejected non-finite float spellings such as `INF`, `Infinity`, and
  `NaN` from HL7 numeric validation.
...
- Target directory: `F:\cargo-target\hl7v2-rs-readiness-numeric-refresh`
- Cargo incremental: `0`
...
| `check-doc-links` | Passed. 160 Markdown files and 371 local links checked. |
| `check-file-policy` | Passed. 519 non-ignored files checked with 39 allowlist entries and 30 companion ledger entries. |
