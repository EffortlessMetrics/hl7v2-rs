...
| Python TestPyPI distribution (`hl7v2`) | Blocked | Hosted non-publishing wheel/import/evidence workflow proof passed on current `main` after the SRP refactor wave; see the [Python TestPyPI non-publish proof](../audits/python-testpypi-nonpublish-proof-2026-05-16.md). The latest publishing-mode attempt again built and smoke-tested the wheel before failing at the external Trusted Publisher boundary; see the [Python TestPyPI publish attempt refresh](../audits/python-testpypi-publish-attempt-2026-05-17.md). Historical issue [#563](https://github.com/EffortlessMetrics/hl7v2-rs/issues/563) is currently closed, but TestPyPI/PyPI package checks still return no public `hl7v2` package and no upload/install-back receipt exists. TestPyPI success still requires upload and install-back receipt. |
