...
| Safe-error and PHI sentinel parity | Stable Rust core, CLI, REST, and gRPC proof; Python local-wheel proof command recorded; TypeScript not released | `test_data/security/safe-error-phi-parity.json`; `cargo test -p hl7v2 --test safe_error_phi_parity --all-features`; `cargo test -p hl7v2-cli --test integration_tests test_parse_safe_error_does_not_emit_manifest_phi_sentinels`; `cargo test -p hl7v2-server --test parse_endpoint_test test_parse_malformed_message_returns_error`; `cargo test -p hl7v2-server --test validate_endpoint_test test_validate_invalid_profile_yaml_returns_error`; `cargo test -p hl7v2-server --test grpc_contract_tests test_grpc_parse_invalid_hl7_returns_parse_error`; `python tests/python_smoke/smoke.py` after local wheel install; [policy/evidence-parity.toml](../../policy/evidence-parity.toml) |
| Schema-version parity | Stable Rust, CLI, REST, and gRPC proof; Python local-wheel proof command recorded; TypeScript not released | `test_data/evidence/schema-version-parity.json`; `cargo test -p hl7v2-test-utils schema_version --locked`; `cargo test -p hl7v2-cli --test integration_tests test_validate_sample_json_schema_version_two --locked`; `cargo test -p hl7v2-server --test validate_endpoint_test test_validate_report_schema_v2_returns_nested_provenance_report --locked`; `cargo test -p hl7v2-server --test grpc_contract_tests test_grpc_validate_separates_errors_from_warnings --locked`; `python tests/python_smoke/smoke.py` after local wheel install; `cargo run -p xtask -- check-evidence-parity` |
...
| REST evidence sidecar | Stable | `cargo test -p hl7v2-server --test validate_endpoint_test`; `cargo test -p hl7v2-server --test validate_redacted_endpoint_test`; `cargo test -p hl7v2-server --test bundle_endpoint_test`; `cargo test -p hl7v2-server --test replay_endpoint_test` |
...
| PHI and quarantine sentinels | Stable | `cargo test -p hl7v2 --test safe_error_phi_parity --all-features`; `cargo test -p hl7v2-e2e-tests security`; `cargo test -p hl7v2-server --test quarantine_output_hooks_test`; [policy/evidence-parity.toml](../../policy/evidence-parity.toml) |
...
| Python TestPyPI distribution (`hl7v2`) | Blocked | Hosted non-publishing wheel/import/evidence workflow proof passed on current `main` after the SRP refactor wave; see the [Python TestPyPI non-publish proof](../audits/python-testpypi-nonpublish-proof-2026-05-16.md). The latest publishing-mode attempt again built and smoke-tested the wheel before failing at the external Trusted Publisher boundary; see the [Python TestPyPI publish attempt refresh](../audits/python-testpypi-publish-attempt-2026-05-17.md). Issue [#563](https://github.com/EffortlessMetrics/hl7v2-rs/issues/563) remains the external Trusted Publisher blocker; TestPyPI success still requires upload and install-back receipt. |
...
[`7a7c60907100442c5bbf5f2e7fcf7c2378035261`](../audits/publish-dry-run-v1.5.0-2026-05-17-current-main-refresh.md)
after the TestPyPI blocker receipt, first-use README routing, testing guide
refresh, issue-template cleanup, and contracts Buf CLI pin landed through
#720. This is not a public Python `hl7v2` TestPyPI/PyPI receipt.
