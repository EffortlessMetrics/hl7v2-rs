...
const BADGE_RIPR: &str = "badges/ripr.json";
const BADGE_RIPR_PLUS: &str = "badges/ripr-plus.json";
const GENERATED_BADGE_RIPR: &str = "target/xtask/badges/ripr.json";
const GENERATED_BADGE_RIPR_PLUS: &str = "target/xtask/badges/ripr-plus.json";
...
pub(crate) fn badges(check: bool) -> Result<()> {
    let repo = repo_root()?;
    let ripr = generate_shields_badge(&repo, "repo-badge-shields", "ripr");
    let ripr_plus = generate_shields_badge(&repo, "repo-badge-plus-shields", "ripr+");
    write_generated_badges(&repo, &ripr, &ripr_plus)?;

    if check {
        check_badge(&repo, BADGE_RIPR, &ripr)?;
        check_badge(&repo, BADGE_RIPR_PLUS, &ripr_plus)?;
        println!("Badge endpoints are current.");
    } else {
        write_badge(&repo, BADGE_RIPR, &ripr)?;
        write_badge(&repo, BADGE_RIPR_PLUS, &ripr_plus)?;
        println!("Wrote {BADGE_RIPR}");
        println!("Wrote {BADGE_RIPR_PLUS}");
    }
    Ok(())
}

pub(crate) fn ripr_pr(root: &str, base: &str, head: &str, check: bool) -> Result<()> {
    let repo = repo_root()?;
...
pub(crate) fn ripr_review_comments(root: &str, base: &str, head: &str, check: bool) -> Result<()> {
    let repo = repo_root()?;
...
pub(crate) fn ripr_pr_summary(check: bool) -> Result<()> {
    let repo = repo_root()?;
    ...
    if check {
        ...
        if actual != summary {
            bail!("{PR_SUMMARY_MD} is stale; run `cargo run -p xtask -- ripr-pr-summary`");
        }
...
pub(crate) fn ripr_annotations(comments: &str, out: &str, check: bool) -> Result<()> {
    let repo = repo_root()?;
    ...
    if check {
        ...
        if actual != generated.text {
            bail!("{out} is stale; run `cargo run -p xtask -- ripr-annotations`");
        }
...
pub(crate) fn impacted_evidence(
    pr_evidence: &str,
    labels: &[String],
    ...
    if check {
        ...
        if actual_json != json_text || actual_md != markdown {
            bail!("impacted evidence is stale; run `cargo run -p xtask -- impacted-evidence`");
        }
...
fn generate_shields_badge(repo: &Path, format: &str, expected_label: &str) -> Value {
    ...
    match run_ripr(&args) {
        Ok(stdout) => match serde_json::from_str::<Value>(&stdout) {
            Ok(value) => match validate_shields_badge(&value, expected_label) {
                Ok(()) => value,
                Err(err) => error_badge(expected_label, &err.to_string()),
            },
            Err(err) => error_badge(expected_label, &format!("invalid badge JSON: {err}")),
        },
        Err(err) => error_badge(expected_label, &err.to_string()),
    }
}

fn validate_shields_badge(value: &Value, expected_label: &str) -> Result<()> {
    if value.get("schemaVersion").and_then(Value::as_u64) != Some(1) {
        bail!("badge schemaVersion must be 1");
    }
    if value.get("label").and_then(Value::as_str) != Some(expected_label) {
        bail!("badge label must be {expected_label}");
    }
    for key in ["message", "color"] {
        ...
        {
            bail!("badge {key} must be a non-empty string");
        }
    }
    Ok(())
}

fn error_badge(label: &str, _message: &str) -> Value {
    json!({
        "schemaVersion": 1,
        "label": label,
        "message": "error",
...
fn write_generated_badges(repo: &Path, ripr: &Value, ripr_plus: &Value) -> Result<()> {
    write_badge(repo, GENERATED_BADGE_RIPR, ripr)?;
    write_badge(repo, GENERATED_BADGE_RIPR_PLUS, ripr_plus)
}

fn write_badge(repo: &Path, relative: &str, value: &Value) -> Result<()> {
    write_text(&repo.join(relative), &format_json(value)?)
}

fn check_badge(repo: &Path, committed: &str, generated: &Value) -> Result<()> {
    let expected = format_json(generated)?;
    let actual = fs::read_to_string(repo.join(committed))
        .with_context(|| format!("missing or unreadable {committed}"))?;
    if actual != expected {
        bail!("{committed} is stale; run `cargo run -p xtask -- badges`");
    }
    Ok(())
}
...
fn write_pr_evidence(repo: &Path, options: &PrEvidenceOptions) -> Result<()> {
    ...
    write_text(
        &repo.join(PR_EVIDENCE_MD),
        &render_pr_evidence_markdown(&packet),
    )?;
...
fn write_error_review_comments(
    ...
    write_text(
        &repo.join(REVIEW_COMMENTS_MD),
        &render_error_review_comments_markdown(&packet),
    )
}
...
fn render_pr_evidence_summary(repo: &Path) -> String {
    ...
    out.push_str(&format!(
        "- requires_targeted_mutation: {}\n",
        option_summary_bool(pr_value, "requires_targeted_mutation")
    ));
    out.push_str(&format!(
        "- ripr_severe_gap: {}\n",
        option_summary_bool(pr_value, "ripr_severe_gap")
    ));
    out.push_str(&format!(
        "- routing_reason: `{}`\n\n",
        option_summary_string_or_null(pr_value, "routing_reason")
    ));
    out.push_str("## Artifacts\n\n");
    out.push_str("| Artifact | Path | State |\n");
    out.push_str("| --- | --- | --- |\n");
    out.push_str(&format!(
        "| PR evidence JSON | `{PR_EVIDENCE_JSON}` | {} |\n",
        pr_evidence.state
    ));
    out.push_str(&format!(
    ...
    out.push_str(&format!(
        "| PR evidence summary Markdown | `{PR_SUMMARY_MD}` | generated |\n"
    ));
...
fn render_annotations(repo: &Path, comments: &str) -> Result<AnnotationOutput> {
    ...
    let comments_array = packet
        .get("comments")
        .and_then(Value::as_array)
        .ok_or_else(|| anyhow!("{comments} is missing comments[]"))?;
    let mut out = String::new();
...
fn annotation_from_comment(item: &Value) -> Result<String> {
    let placement = item
        ...
        .ok_or_else(|| anyhow!("comments[] item is missing placement object"))?;
    let path = string_key(placement, "path")?;
    let line = placement
        .get("line")
        .and_then(Value::as_u64)
        .ok_or_else(|| anyhow!("comments[] placement.line is missing or not an integer"))?;
...
fn string_key(object: &Map<String, Value>, key: &str) -> Result<String> {
    object
        ...
        .map(str::to_string)
        .ok_or_else(|| anyhow!("comments[] placement.{key} is missing or empty"))
}
